...
func main() {
	...
	if err != nil {
		fmt.Printf("Fail to read file: %v", err);os.Exit(1)
	}
	vhosts := cfg.SectionStrings()
	err = cfg.Section("").MapTo(&global)
	if err != nil {
		fmt.Println("Error while loading globals", err);os.Exit(1)
	}
	...
	if err != nil {
		fmt.Println("Error while loading default certs", err);os.Exit(1)
	}
	for _, host := range vhosts {
		...
		if err != nil {
			fmt.Println("Error while loading '"+host+"' certs", err);os.Exit(1)
		}
		if(err!= nil){
